  ...
  function WebSocketRC(props) {
    ...
    _this.handleMessage = function (_ref) {
      ...
      try {
        ...
        var action = json[_this.state.actionKey];
        var handler = _this.state.actionMap[action];
        if (action && handler) {
          handler(json);
        } else {
          _this.state.onMessage(json);
        }
      } catch (e) {
        var result = {
          rawText: data
        };
        _this.state.onMessage(result);
      }
    };
    ...
    value: function setWebSocket() {
      ...
      ws.onopen = function () {
        if (_this2.state.onCreate && ws.readyState === 1) {
          _this2.state.onCreate(proxy, ws);
        }
      };
